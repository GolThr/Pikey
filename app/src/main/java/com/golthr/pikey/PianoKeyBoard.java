...
public class PianoKeyBoard extends View {
    ...
    private ArrayList<PianoKey> list = new ArrayList<>();
    private ArrayList<PianoKey> reverseList = new ArrayList<>();
    private HashMap<Integer, PianoKey> keyMap = new HashMap<>();
    ...
    private void onFingerUp(int index, float x, float y) {
        PianoKey key = this.keyMap.get(Integer.valueOf(index));
        if (key != null) {
            fireKeyUp(key);
            this.keyMap.remove(index);
        } else {
            PianoKey key1 = pointerInWhichKey(x, y);
            fireKeyUp(key1);
        }
    }

    private void onFingerDown(int index, float x, float y) {
        PianoKey key = pointerInWhichKey(x, y);
        fireKeyDown(key);
    ...
    private void onAllFingersUp() {
        Iterator<PianoKey> localIterator = this.keyMap.values().iterator();
    ...
    private void onFingerMove(int index, float x, float y) {
        PianoKey key = this.keyMap.get(Integer.valueOf(index));
        PianoKey currentKey = pointerInWhichKey(x, y);
        if (key != null) {
    ...
    private void fireKeyUp(PianoKey key) {
        if (key != null) {
            if (keyListener != null) {
    ...
    private void fireKeyDown(PianoKey key) {
        if (key != null) {
            if (keyListener != null) {
    ...
    private PianoKey pointerInWhichKey(float x, float y) {
        PianoKey currentKey = null;
        for (PianoKey key : reverseList) {
...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
     ...
     */
    public PianoKey getKeyByKeycode(int code) {
        PianoKey key = null;
        for (PianoKey temp : list) {
